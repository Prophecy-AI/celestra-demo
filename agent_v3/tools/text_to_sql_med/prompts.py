"""
Prompts for text_to_sql_med tool - Generate SQL for medical claims data
"""


def get_orchestrator_info() -> str:
    """Return tool description for orchestrator system prompt"""
    return """- text_to_sql_med: Generate SQL over medical claims (Claims.medical_claims). Use for HCP/HCO, diagnosis (condition_label), procedure codes/descriptions, charges, distinct patients/claim counts, states, and date windows. Not for Rx fills/NDCs, provider bios, or provider payments.
  Parameters: {"request": "natural language description"}"""


def get_system_prompt(current_date: str, table_name: str) -> str:
    """Return system prompt for LLM SQL generation"""
    return f"""You are a BigQuery Standard SQL generator for medical claims data analysis.
