"""
Prompts for text_to_sql_rx tool - Generate SQL for Rx claims data
"""


def get_orchestrator_info() -> str:
    """Return tool description for orchestrator system prompt"""
    return """- text_to_sql_rx: Generate SQL over Rx prescriptions (Claims.rx_claims). Use for drug/NDC/prescriber queries, fill dates, quantities, days supply, payer channels, and date windows. Not for diagnoses/procedures, provider bios, or provider payments.
...
def get_system_prompt(current_date: str, table_name: str) -> str:
    ...
    return f"""You are a BigQuery Standard SQL generator for prescription (Rx) claims data analysis.
