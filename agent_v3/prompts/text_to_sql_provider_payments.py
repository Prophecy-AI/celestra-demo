"""
Prompts for text_to_sql_provider_payments tool - Generate SQL for provider payments data
"""


def get_orchestrator_info() -> str:
    """Return tool description for orchestrator system prompt"""
    return """- text_to_sql_provider_payments: Generate SQL over provider payments (HCP.provider_payments). Use for payments to NPIs by payer_company, associated_product, nature_of_payment, product_type, program_year; totals and breakdowns. Not for Rx/medical claims or provider bios.
...
def get_system_prompt(table_name: str) -> str:
    ...
    return f"""You are a BigQuery Standard SQL generator for Healthcare Providers Payments data analysis.
